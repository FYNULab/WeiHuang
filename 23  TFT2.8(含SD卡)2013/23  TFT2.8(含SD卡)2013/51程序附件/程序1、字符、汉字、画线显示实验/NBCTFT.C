...
void TFT_DrawLine(unsigned int X0, unsigned int Y0, unsigned int X1 ,unsigned int Y1, unsigned int color)//画斜线
{
    int t, distance;      
    ...
    else if( delta_x == 0 )									   //画竖线
    {
        //LCD12864_DrawYline( X0, Y0, Y1, color ) ;
		for(t=((Y1<Y0)?Y1:Y0);t<=((Y1>Y0)?Y1:Y0);t++)
		Put_pixel(X0,t,color);
        //return ;
    }
    else
    {
    ...
    else if(delta_y == 0 )											 //画横线
    {
        for(t=((X1<X0)?X1:X0);t<=((X1>X0)?X1:X0);t++)
		Put_pixel(t,Y0,color);
    }
    else
    {
        incy = -1 ;
...
void draw_circle(uint x0,uint y0,uint radius,uint color)	// 圆心坐标 半径
{
    uint x,y,y_u,y_d ;								// 圆的边界坐标	 分为圆的上半部分和下半部分
	float k;
	uint m;
	int n;
	uint circle_l,circle_r,circle_u,circle_d;    // 圆的左右上下边界
	circle_l=(x0-radius*3/4>0)? (x0-radius*3/4) : 0;
	circle_r=(x0+radius*3/4<240)? (x0+radius*3/4) : 240;
	
	circle_u=(y0-radius*3/4>0)? (y0-radius*3/4) : 0;
	circle_d=(y0+radius*3/4<320)? (y0+radius*3/4) : 320;
		
	for(x=circle_l;x<=circle_r;x++)
	{
	    n=x-x0;
	    k=sqrt((float)(radius*radius-n*n));
	    m=(uint)k;
	    y_u=(y0+m<320)?(y0+m):320;
		y_d=(y0-m>0)?(y0-m):0;

		Put_pixel(x,y_u,color);
		Put_pixel(x,y_d,color);
	
	}
   
	for(y=circle_u;y<=circle_d;y++)
	{
	    n=y-y0;
	    k=sqrt((float)(radius*radius-n*n));
	    m=(uint)k;
	    y_u=(x0+m<240)?(x0+m):240;
		y_d=(x0-m>0)?(x0-m):0;

		Put_pixel(y_u,y,color);
		Put_pixel(y_d,y,color);
